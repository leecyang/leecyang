...
过去一年，我主要在做全栈系统和 AI Infra：从 React / Vue 前端、FastAPI / Spring Boot / Node 服务，到模型接入、推理服务、部署和运行时。日常会涉及 Ascend / CUDA 算子优化、模型迁移与量化、显存问题排查，以及把系统跑在边缘设备上。网络安全和自动化工具开发是其中的补充，我会在 CAS 登录、白名单代理、预约任务和通知推送上处理具体的协议与流程。
